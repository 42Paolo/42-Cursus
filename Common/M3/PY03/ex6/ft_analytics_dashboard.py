class User():
	NAMES = ["alice", "bob", "charlie", "diana", "ethan",
	         "luna", "marco", "sofia", "leo", "nina"]
	REGIONS = ["north", "east", "central", "south", "west"]
	ACHIEVEMENTS = ["first_kill", "level_10", "boss_slayer",
	                "speedrun", "no_damage", "collector"]

	def __init__(self, name: str, region: str, days_not_log: int, score: int):
		self.name = name
		self.region = region
...
def build_users() -> list:
	...
	users[0].achievements = ["first_kill", "level_10", "boss_slayer", "speedrun", "collector"]
	users[1].achievements = ["first_kill", "level_10", "no_damage"]
	users[2].achievements = ["first_kill", "level_10", "boss_slayer", "speedrun",
	                         "no_damage", "collector", "extra"]
